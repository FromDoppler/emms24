...
{
 $sql_query="UPDATE aliados_starter SET `status`='".$_GET['status']."' WHERE id=".$_GET['changestatus_id'];
 mysqli_query($con,$sql_query);
...
{
  window.location.href='edit_aliados_starter.php?edit_id='+id+"&token="+token;
}
function view_id(id, token)
{
  window.location.href='view_aliados_starter.php?view_id='+id+"&token="+token;
}
